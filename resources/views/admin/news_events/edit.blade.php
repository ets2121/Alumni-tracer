<x-layouts.admin>
    <x-slot name="header">
        Edit Post: {{ $newsEvent->title }}
    </x-slot>

    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg max-w-2xl mx-auto">
        <div class="p-6 text-gray-900">
            <form action="{{ route('admin.news_events.update', $newsEvent->id) }}" method="POST"
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $newsEvent->title) }}" required
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="4" required
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-brand-500 focus:border-brand-500 sm:text-sm">{{ old('content', $newsEvent->content) }}</textarea>
                ...
                <div>
                    ...
                    @if($newsEvent->image_path)
                        <div class="mt-2 mb-2">
                            <img src="{{ asset('storage/' . $newsEvent->image_path) }}" alt="Current Image"
                                class="h-32 w-auto object-cover rounded-md border border-gray-200 shadow-sm">
                    ...
                    <div
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <p class="text-xs text-gray-500">
                                PNG, JPG, GIF up to 2MB. Leave blank to keep current.
                            </p>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="datetime-local" name="event_date" id="event_date"
                            value="{{ old('event_date', $newsEvent->event_date ? $newsEvent->event_date->format('Y-m-d\TH:i') : '') }}"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-brand-500 focus:border-brand-500 sm:text-sm">
