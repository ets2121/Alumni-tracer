                ...
                <tr>
                    <td class="px-5 py-5 border-b border-gray-200 dark:border-dark-border bg-white dark:bg-dark-bg text-sm">
                        <div class="flex flex-col">
                            <span
                                class="font-bold text-gray-900 dark:text-dark-text-primary">{{ $memo->memo_number }}</span>
                            <span class="text-gray-600 dark:text-dark-text-secondary font-medium">{{ $memo->title }}</span>
                        </div>
                    </td>
                    <td class="px-5 py-5 border-b border-gray-200 dark:border-dark-border bg-white dark:bg-dark-bg text-sm">
                        <div class="flex flex-col gap-1">
                            <span
                                class="inline-flex px-2 text-xs font-semibold leading-5 text-green-800 dark:text-green-300 bg-green-100 dark:bg-green-900/30 rounded-full w-fit">
                                {{ $memo->category }}
                            </span>
                            @if($memo->description)
                                <p class="text-gray-500 dark:text-dark-text-muted text-xs mt-1">
                                    {{ Str::limit($memo->description, 80) }}</p>
                            @endif
                        </div>
                    </td>
                    <td
                        class="px-5 py-5 border-b border-gray-200 dark:border-dark-border bg-white dark:bg-dark-bg text-sm text-gray-500 dark:text-dark-text-muted whitespace-nowrap">
                        ...
                        <div class="flex items-center gap-3">
                            @if($memo->file_path)
                                <a href="{{ asset('storage/' . $memo->file_path) }}" target="_blank"
                                    class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300 font-medium transition-colors">View</a>
                            @endif
                            <button @click="openModal('{{ route('admin.memos.edit', $memo->id) }}', 'Edit Memo')"
                                class="text-brand-600 dark:text-brand-400 hover:text-brand-900 dark:hover:text-brand-300 font-medium transition-colors">Edit</button>
                            <button
