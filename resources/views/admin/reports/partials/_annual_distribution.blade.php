<div class="space-y-8">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            <h2 class="text-lg font-black text-gray-900 uppercase tracking-tighter">Comparative Analytical Matrix</h2>
    ...
    @if($data['distribution']->isEmpty())
        <div class="bg-white p-16 rounded-[3rem] border border-gray-100 shadow-sm text-center">
            ...
            <p class="text-lg font-black text-gray-900 uppercase tracking-tighter">No Analytical Data</p>
            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-2">No records match the current filter selection for this matrix.</p>
        </div>
    @else
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div class="bg-gray-50/50 rounded-[2.5rem] p-8 border border-gray-100">
            <h4 class="text-[10px] font-black text-gray-900 uppercase tracking-widest mb-6">Distribution Summary</h4>
            <div class="space-y-4">
                @foreach($data['distribution']->take(5) as $item)
                    <div class="flex items-center justify-between">
                        <span class="text-[11px] font-bold text-gray-500 uppercase">{{ $item->batch_year ?? $item->label ?? 'Unknown' }}</span>
                        <div class="flex items-center gap-4">
                            ...
                            <span class="text-[11px] font-black text-gray-900">{{ $item->count }}</span>
                        </div>
                    </div>
                @endforeach
            </div>
