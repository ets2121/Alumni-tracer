...
<div class="space-y-8" x-data="{ subTab: 'all' }">
    ...
    <div class="flex items-center justify-between pb-8 border-b-2 border-gray-900 dark:border-dark-border mb-8">
        <div class="flex items-center gap-4">
            <div class="w-12 h-12 bg-brand-600 rounded-xl flex items-center justify-center text-white shadow-lg">
    ...
    <!-- Summary KPI Strip -->
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 animate-in fade-in duration-700">
        <div class="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm transition-all hover:shadow-md">
            <p class="text-2xl font-black text-gray-900 dark:text-dark-text-primary">
                {{ number_format(data_get($summary, 'total_count', 0)) }}
            </p>
        </div>
        <div class="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm transition-all hover:shadow-md">
            <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Employment</p>
            <p class="text-2xl font-black text-brand-600">{{ data_get($summary, 'employment_rate', 0) }}%</p>
        </div>
        <div class="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm transition-all hover:shadow-md">
            <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Overseas</p>
            <p class="text-2xl font-black text-blue-600">{{ data_get($summary, 'overseas_rate', 0) }}%</p>
        </div>
        <div
            class="bg-white dark:bg-dark-bg-elevated p-6 rounded-[2rem] border border-gray-100 dark:border-dark-border shadow-sm transition-all hover:shadow-md col-span-2">
            <p class="text-[9px] font-black text-gray-400 dark:text-dark-text-muted uppercase tracking-widest mb-1">Top
                Field</p>
            <p class="text-lg font-black text-gray-900 dark:text-dark-text-primary truncate">
                {{ data_get($summary, 'dominant_field', 'N/A') }}
            </p>
        </div>
        <div
            class="bg-white dark:bg-dark-bg-elevated p-6 rounded-[2rem] border border-gray-100 dark:border-dark-border shadow-sm transition-all hover:shadow-md">
            <p class="text-[9px] font-black text-gray-400 dark:text-dark-text-muted uppercase tracking-widest mb-1">Data
                Integrity</p>
            <div class="flex items-end gap-2">
                <p
                    class="text-2xl font-black {{ $summary['integrity_score'] > 80 ? 'text-green-600' : 'text-amber-500' }}">
                    {{ data_get($summary, 'integrity_score', 0) }}%
                </p>
                <div class="mb-1">
                    <div class="w-12 h-1.5 bg-gray-100 dark:bg-dark-bg-subtle rounded-full overflow-hidden">
                        <div class="h-full {{ $summary['integrity_score'] > 80 ? 'bg-green-500' : 'bg-amber-500' }}"
                            style="width: {{ data_get($summary, 'integrity_score', 0) }}%"></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div
        ...
        <div class="flex bg-gray-100 dark:bg-dark-bg-subtle p-1 rounded-2xl w-full md:w-auto">
            <button @click="subTab = 'all'"
                :class="subTab === 'all' ? 'bg-white dark:bg-dark-bg text-gray-900 dark:text-dark-text-primary shadow-sm' : 'text-gray-500 dark:text-dark-text-muted hover:text-gray-700 dark:hover:text-dark-text-secondary'"
            ...
            <button @click="subTab = 'tracer'"
                ...
                class="flex-1 md:flex-none px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Tracer
                Results</button>
        </div>
    ...
    <div x-show="subTab === 'program'" class="animate-in fade-in duration-500">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div
                class="lg:col-span-2 bg-gray-50 dark:bg-dark-bg-subtle/50 p-8 rounded-[2.5rem] border border-gray-100 dark:border-dark-border">
                <h4 class="text-sm font-black text-gray-900 dark:text-dark-text-primary mb-6 uppercase tracking-tight">
            ...
            <div class="space-y-4">
                <div class="bg-brand-600 p-6 rounded-[2rem] text-white">
                    ...
                    <h5 class="text-xl font-black uppercase leading-tight">
                        {{ data_get($data, 'by_course.0.name', 'N/A') }}
                    </h5>
                </div>
                <!-- Batch Growth Summary -->
                <div
                    class="bg-white dark:bg-dark-bg-elevated p-6 rounded-[2rem] border border-gray-100 dark:border-dark-border shadow-sm">
                    <h5
                        class="text-[9px] font-black text-gray-400 dark:text-dark-text-muted uppercase tracking-widest mb-4">
                        Batch Volumetrics
                    </h5>
                    <div class="space-y-3">
                        @foreach(data_get($data, 'by_batch', []) as $batch)
                            <div class="flex justify-between items-center text-xs">
                                <span class="font-bold text-gray-500 dark:text-dark-text-secondary">Class of
                                    {{ $batch->batch_year }}</span>
                                <span
                                    class="font-black text-gray-900 dark:text-dark-text-primary">{{ $batch->count }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
    ...
    <div x-show="subTab === 'labor'" class="space-y-8 animate-in fade-in duration-500">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <!-- Location Matrix (Grouped) -->
            <div
                class="bg-indigo-50 dark:bg-indigo-900/10 p-8 rounded-[2.5rem] border border-indigo-100 dark:border-indigo-900/30 md:col-span-1">
                <h4 class="text-xs font-black text-indigo-900 dark:text-indigo-300 mb-6 uppercase tracking-tight">
                    Location Matrix (Grouped)
                </h4>
                <div class="h-48 w-full">
            ...
            <div
                class="bg-purple-50 dark:bg-purple-900/10 p-8 rounded-[2.5rem] border border-purple-100 dark:border-purple-900/30 md:col-span-1">
                <h4 class="text-xs font-black text-purple-900 dark:text-purple-300 mb-6 uppercase tracking-tight">Sector
                    Ranking (Grouped)
                </h4>
                <div class="h-48 w-full">
            ...
            <!-- Industry Bar (Horizontal) -->
            <div
                class="bg-amber-50 dark:bg-amber-900/10 p-8 rounded-[2.5rem] border border-amber-100 dark:border-amber-900/30">
                <h4 class="text-xs font-black text-amber-900 dark:text-amber-300 mb-6 uppercase tracking-tight">Top
                    Field Clusters</h4>
                <div class="h-48 w-full">
        ...
        <div
            class="bg-gray-50 dark:bg-dark-bg-subtle/50 p-8 rounded-[2.5rem] border border-gray-100 dark:border-dark-border mb-8">
            <h4 class="text-sm font-black text-gray-900 dark:text-dark-text-primary mb-6 uppercase tracking-tight">
                Employment Status Concentration
            </h4>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div
                class="bg-green-50 dark:bg-green-900/10 p-6 rounded-[2rem] border border-green-100 dark:border-green-900/30">
                ...
                @php
                    $dEmployment = collect(data_get($data, 'by_employment', []));
                    $total = $dEmployment->sum('count');
                    $employed = $dEmployment->where('employment_status', 'Employed')->first()->count ?? 0;
            ...
            <div
                class="bg-white dark:bg-dark-bg-elevated p-6 rounded-[2rem] border border-gray-100 dark:border-dark-border shadow-sm flex items-center gap-6">
                <div class="flex-1">
                    <h5
                        class="text-[10px] font-black text-gray-400 dark:text-dark-text-muted uppercase tracking-widest mb-2">
                        Social Mobility
                        Index</h5>
                    <p class="text-xs text-gray-500 dark:text-dark-text-secondary font-medium leading-relaxed italic">
                        Analysis of career progression
                        and professional transitions within the alumni community based on active registry data.</p>
                </div>
                <div class="p-4 bg-gray-50 dark:bg-dark-bg-subtle rounded-2xl"><svg
                        class="w-8 h-8 text-gray-300 dark:text-dark-text-disabled" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div
                class="bg-gray-50/50 dark:bg-dark-bg-subtle/50 p-6 rounded-[2rem] border border-gray-100 dark:border-dark-border flex flex-col items-center">
                <span
                    class="text-[9px] font-black text-gray-400 dark:text-dark-text-muted uppercase tracking-widest mb-4">Work
                    Status</span>
                <div class="h-32 w-32 relative">
                    <canvas id="chartByWorkStatus"
                        data-labels="{{ json_encode(collect(data_get($data, 'by_work_status'))->pluck('work_status')) }}"
                        data-values="{{ json_encode(collect(data_get($data, 'by_work_status'))->pluck('count')) }}"></canvas>
                </div>
            ...
            <div
                class="bg-gray-50/50 dark:bg-dark-bg-subtle/50 p-6 rounded-[2rem] border border-gray-100 dark:border-dark-border flex flex-col items-center">
                <span
                    class="text-[9px] font-black text-gray-400 dark:text-dark-text-muted uppercase tracking-widest mb-4">Sector
                    Distribution</span>
                <div class="h-32 w-32 relative">
                    <canvas id="chartByEstablishment"
                        data-labels="{{ json_encode(collect(data_get($data, 'by_establishment'))->pluck('establishment_type')) }}"
                        data-values="{{ json_encode(collect(data_get($data, 'by_establishment'))->pluck('count')) }}"></canvas>
                </div>
            ...
            <div
                class="bg-gray-50/50 dark:bg-dark-bg-subtle/50 p-6 rounded-[2rem] border border-gray-100 dark:border-dark-border flex flex-col items-center">
                <span
                    class="text-[9px] font-black text-gray-400 dark:text-dark-text-muted uppercase tracking-widest mb-4">Geographic
                    Scope</span>
                <div class="h-32 w-32 relative">
                    <canvas id="chartByWorkLocation"
                        data-labels="{{ json_encode(collect(data_get($data, 'by_work_location'))->pluck('work_location')) }}"
                        data-values="{{ json_encode(collect(data_get($data, 'by_work_location'))->pluck('count')) }}"></canvas>
                </div>
            ...
            <div
                class="bg-gray-50/50 dark:bg-dark-bg-subtle/50 p-6 rounded-[2rem] border border-gray-100 dark:border-dark-border flex flex-col items-center">
                <span
                    class="text-[9px] font-black text-gray-400 dark:text-dark-text-muted uppercase tracking-widest mb-4">Industry
                    Pulse</span>
    ...
    <div x-show="subTab === 'demographics'" class="animate-in fade-in duration-500">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div
                class="bg-gray-50 dark:bg-dark-bg-subtle/50 p-8 rounded-[2.5rem] border border-gray-100 dark:border-dark-border">
                <h4
                    class="text-xs font-black text-gray-400 dark:text-dark-text-muted mb-6 uppercase tracking-widest px-2">
                    Gender Demographics
                    (Enhanced)</h4>
    ...
    <div x-show="subTab === 'tracer'" class="animate-in fade-in duration-500">
        <div
            class="bg-white dark:bg-dark-bg-elevated p-12 rounded-[3rem] border border-gray-100 dark:border-dark-border shadow-xl text-center">
            <div
                class="w-20 h-20 bg-brand-50 text-brand-600 rounded-[2.5rem] flex items-center justify-center mx-auto mb-8">
                <svg class="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
            </div>
            <h3 class="text-3xl font-black text-gray-900 dark:text-dark-text-primary mb-4 uppercase tracking-tighter">
                Graduate Tracer Engine</h3>
            <p class="text-gray-500 dark:text-dark-text-secondary max-w-xl mx-auto mb-10 leading-relaxed font-medium">
                Capture Likert-scale sentiment
                and multiple-choice career path analysis. CHED-compliant reporting for institutional evaluation.</p>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="bg-gray-50 p-8 rounded-[2.5rem] border border-gray-100">
                    <h4 class="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-6">Likert Evaluation
                        (Rating)</h4>
                    <div class="h-48 w-full">
                        <canvas id="chartTracerLikert"
                            data-labels='["Very Satisfied", "Satisfied", "Neutral", "Unsatisfied", "Very Unsatisfied"]'
                            data-values='[45, 30, 15, 7, 3]'></canvas>
                    </div>
                </div>
                <div
                    class="bg-gray-50 dark:bg-dark-bg-subtle/50 p-8 rounded-[2.5rem] border border-gray-100 dark:border-dark-border">
                    <h4
                        ...
                        (Bar)</h4>
                    <div class="h-48 w-full">
                        <canvas id="chartTracerMultiple"
                            data-labels='["Curriculum", "Facilities", "Placement", "Services"]'
                            data-values='[80, 65, 90, 75]'></canvas>
                    </div>
                </div>
            </div>
        </div>
