    ...
    <div x-data="groupManager()" x-init="init()" x-cloak class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 font-medium">Manage batch, course, and general alumni conversation rooms.</p>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" id="groups-grid">
            <template x-for="group in filteredGroups" :key="group.id">
                <div class="bg-white border border-gray-100 rounded-[32px] p-6 hover:shadow-2xl transition-all duration-500 group relative flex flex-col h-full hover:border-brand-200">
                    <!-- Decor -->
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex items-center gap-2 mb-4">
                            <span class="text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full border border-gray-100 text-gray-400 group-hover:border-brand-200 group-hover:text-brand-600 transition-colors" x-text="group.type"></span>
                            <span x-show="group.is_private" class="text-[10px] bg-amber-50 text-amber-600 px-2 py-1 rounded-full font-black uppercase tracking-tighter">Private</span>
                        </div>
                        <p class="text-sm text-gray-500 line-clamp-3 mb-6 leading-relaxed font-medium" x-text="group.description || 'System generated channel for community discussion.'"></p>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3 pt-6 border-t border-gray-50 mt-auto">
                        <div class="bg-gray-50 rounded-2xl p-3 group-hover:bg-brand-50 transition-colors">
                            <p class="text-[10px] text-gray-400 font-black uppercase tracking-tighter mb-1">Potential</p>
                            <p class="text-lg font-black text-gray-900 group-hover:text-brand-600 transition-colors" x-text="group.members_count"></p>
                        </div>
        ...
        <!-- Empty State -->
        <div x-show="filteredGroups.length === 0" class="text-center py-24 bg-white rounded-[40px] border border-gray-100 shadow-sm">
            <div class="w-24 h-24 bg-brand-50 rounded-full flex items-center justify-center mx-auto mb-6 text-brand-600">
                <svg class="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
            </div>
            <h4 class="text-xl font-black text-gray-900">No matching channels found</h4>
            <p class="text-gray-500 max-w-sm mx-auto mt-2 font-medium">Try adjusting your search query or type filters to see other rooms.</p>
        </div>
        ...
        <div x-show="modalOpen" class="fixed inset-0 z-[100] overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen px-4 py-12 text-center sm:p-0">
                <div x-show="modalOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" @click="closeModal()" class="fixed inset-0 transition-opacity bg-gray-900/60 backdrop-blur-sm" aria-hidden="true"></div>
                
                <div x-show="modalOpen" x-transition:enter="ease-out duration-500" x-transition:enter-start="opacity-0 translate-y-20 scale-95" x-transition:enter-end="opacity-100 translate-y-0 scale-100" 
                    ...
                    <div class="p-8 border-b border-gray-50 flex justify-between items-center bg-gray-50/50">
                        <div>
                            ...
                            <p class="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Configure Room Settings</p>
                        </div>
                        <button @click="closeModal()" class="p-2 text-gray-400 hover:text-gray-600 hover:bg-white rounded-2xl transition-all shadow-sm">
