    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('analyticsDashboard', (initData) => ({
            ...
            applyFilters(form) {
                ...
                // We check if we are inside the modal by looking for a parent property or similar
                // But easier: check if the URL didn't change (SPA feel) or just check window location
                // actually, let's just let the event bubble. If no one catches it, we do manual reload?
                // The Modal parent will catch 'filters-applied'.

                // If we are on the standalone page, we need to reload.
                // We can check if we are inside the modal wrapper.
                ...
                this.$nextTick(() => {
                    this.analytics.forEach(item => {
                        if (['radio', 'checkbox', 'scale'].includes(item.type) && this.hasStats(item)) {
                            ...
                            // Improved Colors with Gradients
                            ...
                            let borderColors = 'transparent'; // Default

                            // Dynamic Chart Selection & Coloring
                            if (item.type === 'radio') {
                                chartType = 'doughnut';
                                legendDisplay = true;
                                ...
                                // Ensure standard labels 1-5 even if no data for some?
                                // Ideally yes, but for now map existing.
                                        ...
                                        tooltip: {
                                            ...
                                            callbacks: {
                                                label: function (c) {
                                                    ...
                                                    // For bar charts, total needs to be calculated manually if not stacked
